...
let chronoAnimation
let bugSpeed = 2;
let spriteSheetFilename = ["Fly.png"]
let spriteSheets = [];
let animations = [];
...
spriteSheets[i] = loadImage("Flies/" + spriteSheetFilename[i]);
}
}
...
if (this.xDirection === 1) {
  // Original direction
  scale(this.xDirection, -1, 1); // No scaling on the Y-axis
} else {
  // Flip horizontally for the opposite direction
  scale(1, 1, 1); // Flip vertically by setting the Y-axis scaling factor to -1
}
